...
function parseGraph(raw) {
  ...
  return { nodes: [...nodeIds].map(id => ({ id, x: 0, y: 0 })), edges };
}
...
function runForceLayout(graph, width, height) {
  ...
  const referenceRadius = Math.max(...graph.nodes.map(node => nodeRadius(node.id)), 17);
...
function resolveNodeOverlaps(graph, width, height) {
  ...
  const radii = graph.nodes.map(node => nodeRadius(node.id));
  ...
  for (let step = 0; step < iterations; step++) {
    ...
    graph.nodes.forEach(node => {
      const margin = nodeRadius(node.id) + 12;
  ...
  for (let step = 0; step < iterations; step++) {
    ...
    graph.edges.forEach((edge, edgeIndex) => {
      ...
      graph.nodes.forEach((node, nodeIndex) => {
        ...
        let distance = Math.hypot(dx, dy);
        const clearance = nodeRadius(node.id) + 11;
...
function buildEdgeLayouts(edges) {
  ...
  edges.forEach((edge, index) => {
    ...
    const ordered = selfLoop || compareNodeIds(edge.source, edge.target) <= 0
...
function renderGraph() {
  ...
  graph.nodes.forEach(node => {
    const group = svgElement("g", { class: "graph-node", transform: `translate(${node.x} ${node.y})`, "data-node-id": node.id, tabindex: "0", role: "button", "aria-label": `节点 ${node.id}` });
    group.append(svgElement("circle", { class: "node-hit", r: nodeRadius(node.id) }));
    group.append(svgElement("circle", { class: "node-selection-ring", r: nodeRadius(node.id) + 7 }));
    group.append(svgElement("circle", { class: "node-circle", r: nodeRadius(node.id) }));
    group.append(svgElement("text", { class: "node-label" }, node.id));
...
function updateEdge(path, source, target, directed, gap = 0, layout = null) {
  ...
  const distance = Math.max(1, Math.hypot(dx, dy));
  const ux = dx / distance, uy = dy / distance;
  const sourceR = nodeRadius(source.id);
  const targetR = nodeRadius(target.id) + (directed ? 7 : 0);
...
function organizeAsTree(recordHistory = true, targetCenterOverride = null) {
  ...
  const fallbackRootUsed = !byId.has(requestedRootId);
  const rootId = fallbackRootUsed
    ? graph.nodes.map(node => node.id).sort(compareNodeIds)[0]
    : requestedRootId;
  if (fallbackRootUsed) {
  ...
  const pathEndpointIds = isLongPath
    ? graph.nodes.filter(node => adjacency.get(node.id).length === 1).map(node => node.id).sort(compareNodeIds)
    : [];
  ...
  const referenceSpacing = Math.max(...graph.nodes.map(node => nodeRadius(node.id)), 17)
    * 2 * GENERATED_EDGE_DIAMETER_MULTIPLIER * TREE_LAYOUT_SPACING_SCALE;
  const maximumDiameter = Math.max(...graph.nodes.map(node => nodeRadius(node.id) * 2), 34);
  ...
  elements.status.textContent = isLongPath
    ? `单链已从内部根节点 ${rootId} 向两侧居中分层 · ${layers.length} 层 · 参考边长 ${referenceSpacing.toFixed(1)}`
    : `已按根节点 ${rootId} 父子居中整理 · ${layers.length} 层 · 参考边长 ${referenceSpacing.toFixed(1)}`;
  if (fallbackRootUsed) elements.selection.textContent = `根节点 ${requestedRootId} 不存在，已自动使用最小编号节点 ${rootId}`;
  elements.statusDot.classList.add("ready");
  hideError();
}
...
function layoutPathSerpentine(nodes, rect) {
  ...
  const referenceSpacing = Math.max(...nodes.map(node => nodeRadius(node.id)), 17)
    * 2 * GENERATED_EDGE_DIAMETER_MULTIPLIER * TREE_LAYOUT_SPACING_SCALE;
...
function organizeGraphOnLattice(graph) {
  ...
  const edgeLength = Math.max(...graph.nodes.map(node => nodeRadius(node.id)), 17)
...
function layoutTreeComponent(component, rootId, targetCenter) {
  ...
  while (current.length) {
    ...
    const next = [];
    current.forEach(id => {
      const neighbors = [...component.adjacency.get(id)].sort(compareNodeIds);
  ...
  const referenceSpacing = Math.max(...component.nodes.map(node => nodeRadius(node.id)), 17)
    * 2 * GENERATED_EDGE_DIAMETER_MULTIPLIER * TREE_LAYOUT_SPACING_SCALE;
  const maximumDiameter = Math.max(...component.nodes.map(node => nodeRadius(node.id) * 2), 34);
  const horizontalGap = Math.max(maximumDiameter + 14, referenceSpacing * .78);
  const verticalGap = Math.max(maximumDiameter + 18, referenceSpacing * .9);
  const rawX = centeredTreeXPositions(rootId, children, horizontalGap);
...
function organizeTreeForest(recordHistory = true) {
  ...
  components.forEach(component => {
    const rootId = component.ids.includes(requestedRoot)
      ? requestedRoot
      : [...component.ids].sort(compareNodeIds)[0];
...
function addNodeAt(point) {
  ...
  const id = nextNodeId();
  state.graph.nodes.push({ id, x: point.x, y: point.y });
  if (autoTree && existingNodes.length) {
    ...
    const referenceSpacing = Math.max(...existingNodes.map(node => nodeRadius(node.id)), 17)
      * 2 * GENERATED_EDGE_DIAMETER_MULTIPLIER * TREE_LAYOUT_SPACING_SCALE;
    if (parentDistance > referenceSpacing * NEW_TREE_DISTANCE_FACTOR) {
      commitGraphEdit(`点击位置距离现有树较远，已创建新树根节点 ${id}`);
      return;
    }
    ...
    commitGraphEdit(
      weight === null
        ? `已创建节点 ${id}，并自动连接到父节点 ${parent.id}`
        : `已创建节点 ${id}，自动连接到父节点 ${parent.id} · 边权 ${weight}`,
      { treeCenter: previousCenter }
    );
    return;
  }
  commitGraphEdit(`已创建节点 ${id}`);
}
...
function selectNodeForEdge(nodeId) {
  ...
  if (state.edgeStart === null) {
    ...
    elements.selection.textContent = `已选择起点 ${nodeId}，请点击另一个节点`;
    return;
  }
  ...
  commitGraphEdit(weight === null
    ? `已连接节点 ${source} 和 ${nodeId}${parallelCount > 1 ? ` · 当前共 ${parallelCount} 条重边` : ""}`
    : `已连接节点 ${source} 和 ${nodeId}，随机边权为 ${weight}${parallelCount > 1 ? ` · 当前共 ${parallelCount} 条重边` : ""}`);
}
...
function beginDrawNodeHold(event, nodeElement) {
  ...
  elements.selection.textContent = `短按节点 ${node.id} 连线，长按可移动`;
}
...
function deleteEdge(edgeIndex) {
  ...
  commitGraphEdit(`已删除连线 ${edge.source} — ${edge.target}`);
}
...
function openEdgeWeightEditor(edgeIndex) {
  ...
  document.querySelectorAll(".graph-node.selected").forEach(node => node.classList.remove("selected"));
  elements.edgeWeightDescription.textContent = `${edge.source} — ${edge.target}`;
...
function saveEditedEdgeWeight(event) {
  ...
  commitGraphEdit(nextWeight === null
    ? `已清除边 ${edge.source} — ${edge.target} 的权重`
    : `已将边 ${edge.source} — ${edge.target} 的权重改为 ${nextWeight}`);
  saveSettings();
}
...
function openNodeIdEditor(nodeId) {
  ...
  elements.nodeIdDescription.textContent = `当前编号：${nodeId}`;
  elements.nodeIdInput.value = nodeId;
...
function applyNodeIdMap(idMap) {
  ...
  state.boxSelectedNodeIds = new Set(
    [...state.boxSelectedNodeIds].map(id => idMap.get(id) ?? id)
  );
  if (state.mobileAnimatedSelectedNodeId !== null) {
    state.mobileAnimatedSelectedNodeId =
      idMap.get(state.mobileAnimatedSelectedNodeId) ?? state.mobileAnimatedSelectedNodeId;
  }
  if (state.selectedNode !== null) {
    state.selectedNode = idMap.get(state.selectedNode) ?? state.selectedNode;
  }
  const currentRoot = elements.rootInput.value.trim();
  if (idMap.has(currentRoot)) elements.rootInput.value = idMap.get(currentRoot);
}
...
function saveEditedNodeId(event) {
  ...
  const renameAllToLetters = oldId === "1" && nextId === "A";
  let validationMessage = "";
  if (!nextId) validationMessage = "节点编号不能为空";
  else if (/[\s,]/.test(nextId)) validationMessage = "节点编号不能包含空格或逗号";
  else if (!renameAllToLetters && state.graph.nodes.some(candidate => candidate !== node && candidate.id === nextId)) {
    validationMessage = `节点编号 ${nextId} 已存在`;
  }
  ...
  if (renameAllToLetters) {
    ...
    const idMap = new Map([[oldId, "A"]]);
    orderedOthers.forEach((candidate, index) => {
      idMap.set(candidate.id, alphabeticNodeId(index + 1));
    });
    applyNodeIdMap(idMap);
    message = `已将 ${state.graph.nodes.length} 个节点依次重编号为字母`;
  } else {
    applyNodeIdMap(new Map([[oldId, nextId]]));
    message = `已将节点 ${oldId} 的编号修改为 ${nextId}`;
  }
  state.edgeStart = null;
  elements.nodeIdDialog.close();
...
function beginDeleteGesture(event, target) {
  ...
  state.deleteHoldTimer = window.setTimeout(() => {
    ...
    if (target.nodeId) {
      ...
      updateAllEdges();
      elements.selection.textContent = `正在移动节点 ${target.nodeId}`;
      return;
    }
...
function updateDataInput() {
  ...
  const edgeLines = state.graph.edges.map(edge =>
    [edge.source, edge.target, edge.weight].filter(value => value !== null && value !== undefined).join(" "));
  if (document.body.dataset.mobileStandalone === "true") {
    // 移动版保持原有逐节点、逐边格式。
    elements.input.value = [...nodeLines, ...edgeLines].join("\n");
    return;
  }
  const connectedNodeIds = new Set(state.graph.edges.flatMap(edge => [edge.source, edge.target]));
...
elements.canvasWrap.addEventListener("pointerdown", event => {
  ...
  if (canvasObject) {
    ...
    elements.selection.textContent = `已选择节点 ${state.selectedNode}`;
  } else {
    clearTextObjectSelection();
    ...
    if (moved) {
      ...
      if (state.treeAutoArrange && isForestGraph(state.graph)) {
        ...
        elements.selection.textContent = movedNodeCount > 1
          ...
          : `已移动节点 ${state.draggingNode.id} · 已恢复自动整理`;
      } else {
        elements.selection.textContent = movedNodeCount > 1
          ? `已同时移动 ${movedNodeCount} 个所选节点`
          : `已移动节点 ${state.draggingNode.id}`;
      }
    }
  }
